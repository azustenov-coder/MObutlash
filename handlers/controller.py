...
@router.message(F.text == "Kutilayotgan zayavkalar 📥")
...
async def approve_request(callback: CallbackQuery):
    ...
    role_display = "Super Admin 👑" if user['role'] == 'super_admin' else "Boshqaruvchi 💼"
    summary = (
        f"✅ **Zayavka №{request_id} {role_display} tomonidan tasdiqlandi va yetkazishga yuborildi.**\n"
    ...
    try:
        ...
        await bot.send_message(
            req['created_by'],
            f"✅ Sizning №{request_id}-sonli zayavkangiz tasdiqlandi va kuryerlarga yuborildi."
        )
    except Exception as e:
...
async def reject_request(callback: CallbackQuery):
    ...
    role_display = "Super Admin 👑" if user['role'] == 'super_admin' else "Boshqaruvchi 💼"
    await callback.message.edit_text(
        f"❌ **Zayavka №{request_id} {role_display} tomonidan rad etildi.**\n"
        f"📋 Tavsif: {req['description']}\n"
    ...
    try:
        ...
        await bot.send_message(
            req['created_by'],
            f"❌ Afsuski, sizning №{request_id}-sonli zayavkangiz {role_display} tomonidan rad etildi."
        )
    except Exception as e:
        print(f"Yaratuvchini ogohlantirishda xato: {e}")
